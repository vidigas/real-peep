...
export function Button({
  ...
  // Hierarchy classes for default state
  const hierarchyClasses = {
    primary: destructive 
      ? 'bg-error-500 text-white border-error-500 hover:bg-error-600 active:bg-error-700' 
      : 'bg-primary-500 text-white border-primary-500 hover:bg-primary-600 active:bg-primary-700',
    'secondary-color': destructive
      ? 'bg-white text-error-500 border-error-500 hover:bg-error-50 active:bg-error-100'
      : 'bg-white text-primary-500 border-primary-500 hover:bg-primary-50 active:bg-primary-100',
    'link-color': destructive
      ? 'bg-transparent text-error-500 border-transparent hover:bg-error-50 active:bg-error-100'
      : 'bg-transparent text-primary-500 border-transparent hover:bg-primary-50 active:bg-primary-100',
    'link-gray': 'bg-transparent text-gray-500 border-transparent hover:bg-gray-50 active:bg-gray-100',
    'tertiary-color': destructive
      ? 'bg-error-50 text-error-500 border-error-50 hover:bg-error-100 active:bg-error-200'
      : 'bg-primary-50 text-primary-500 border-primary-50 hover:bg-primary-100 active:bg-primary-200',
  ...
  const disabledClasses = 'bg-gray-100 text-gray-300 border-gray-100 cursor-not-allowed';
  ...
  return (
    <button
      className={cn(
        ...
        // Focus ring colors
        !isDisabled && (destructive ? 'focus:ring-error-500' : 'focus:ring-primary-500'),
    ...
    >
      {icon && iconPosition === 'left' && (
        <span className={cn(iconClasses[size])}>
          {icon}
        </span>
      )}
...
export function PrimaryButton({ children, ...props }: Omit<ButtonProps, 'hierarchy'>) {
  return <Button hierarchy="primary" {...props}>{children}</Button>;
}

export function SecondaryButton({ children, ...props }: Omit<ButtonProps, 'hierarchy'>) {
  return <Button hierarchy="secondary-color" {...props}>{children}</Button>;
}

export function LinkButton({ children, ...props }: Omit<ButtonProps, 'hierarchy'>) {
  return <Button hierarchy="link-color" {...props}>{children}</Button>;
}

export function TertiaryButton({ children, ...props }: Omit<ButtonProps, 'hierarchy'>) {
  return <Button hierarchy="tertiary-color" {...props}>{children}</Button>;
}

// Icon component for buttons (circular icon as shown in design)
